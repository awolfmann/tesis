...
\chapter{Introducción}
  ...
  \section {Motivación}
    ...
    Como imagen de estilo se eligio una imagen abstracta \label{fig:candy} y como imagen de contenido una fotografia de un paisaje \label{fig:tower}, obteniendo como resultado de la
    ...
    \begin{figure}[h]
      ...
      \subfloat[Imagen de Estilo]{\includegraphics[height=0.3\textheight]{./img/jhonson_style_candy.jpg}\label{fig:candy}}

      \subfloat[Imagen de Contenido]{\includegraphics[height=0.3\textheight]{./img/jhonson_content_tower.jpg}\label{fig:tower}}
...
\fi
	...
	\subsubsection{Retropropagación del Error}
	  ...
	  La mayoría de los algoritmos de aprendizaje implican un procedimiento iterativo para la minimización de una función de error, que van ajustand los pesos mediante 
	  ...
	  respecto a los pesos deben ser evaluados. La importancia de la contribución de la retropropagación es la de proveer un método	computacionalmente eficiente para la evaluación 
	  de dichas derivadas. Debido a que en esta etapa los errores son propagados hacia atrás a través de la red, se utiliza el término de retropropagación del error.
	  En la segunda etapa, las derivadas son utilizadas para calcular los ajustes que se les deben hacer a los pesos, en cada una de las capas de la red.
	  Las principales funciones de error de interes, para calcular el error total en todo el conjunto de datos de $N$ instancias, lo hacen midiendo el error en cada una de las instancias,
	  ...
	  \end{equation}
	  Substituyendo \eqref{notation} y \eqref{eq:sustitution1} en \eqref{eq:partial_derivatives} obtenemos:
	  ...
	  Por lo tanto, con el fin de evaluar los derivados, sólo tenemos que calcular el valor de $\delta_j$ para cada oculta y de salida de la unidad en la red y, a continuación, aplicar (5.53). 
	  Como ya hemos visto, para la salida de las unidades, tenemos
	  \FIXME{SEGUIR EN PAGINA 261}
	  Equation (5.53) tells us that the required derivative is obtained simply by multiplying the value of $\delta$ for the unit at the output end of the weight by the value of z for 
	  the unit at the input end of the weight (where z = 1 in the case of a bias). Note that this takes the same form as for the simple linear model considered at the start of 
	  this section. Thus, in order to evaluate the derivatives, we need only to calculate the value of $\delta_j$ for each hidden and output unit in the network, and then apply (5.53). 
	  As we have seen already, for the output units, we have




	  Retropropagación (en inglés Backpropagation) es una forma de computar los gradientes de expresiones mediante la aplicación recursiva de la regla de la cadena.
	  El ciclo consta de 2 fases, propagación y luego actualización de parámetros. Cuando ingresan los parámetros a la función, los valores intermedios se van propagando hasta 
	  calcular el resultado final, esto conforma la fase de propagación. Una vez finalizada la propagación, se compara el resultado obtenido con el resultado deseado. 
	  Luego aplicando la función de pérdida se calcula el error. Mediante el método de optimización (por ejemplo Descenso por el gradiente estocastico), se define el gradiente 
	  para finalmente realizar la retropropagación en la función hasta actualizar los parámetros.
