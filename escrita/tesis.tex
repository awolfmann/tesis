...
%https://en.wikipedia.org/wiki/Machine_learning
	...
	\paragraph{Modelos Generativos vs. Discriminantes}
	  \begin{itemize}
	    \item Para modelos estadísticos
	    \item Calcular la probablidad de la clase objetivo dada las features de entrada
	    \item Podemos realizar este cálculo si tenemos un modelado probabilístico de la probabilidad conjunta de entradas y la clase objetivo
	    \item Sin embargo, no es un requerimiento el modelado de la probabilidad conjunta
	    \item Simulación vs. emulación
	  \end{itemize}

	\paragraph{Modelos Generativos}
	  \begin{itemize}
	    \item Calcular P(y|x1, …, xn) via P(x1, …, xn, y)
	    \item El cálculo de la probabilidad conjunta habilita sistemas reversibles
	    \item Cualquier variable puede ser una clase objetivo
	    \item Requiere un “historia generativa” de como los datos existen y se interrelacionan
	    \item Dependencia entre variables
	    \item Necesitan más datos y/o hacen uso menos eficiente de los mismos
	    \item Si sólo nos interesa la clase objetivo, estamos modelando de más
	  \end{itemize}
