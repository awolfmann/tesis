...
\chapter{Introducción}
  \section {Contexto}
    ...
    \begin{figure}[h]
      \includegraphics[scale=0.5]{./img/nvidia_car_detection.png}\\
  ...
  \section {Motivación}
    ...
    \FIXME{ACOMODAR IMAGENES Y AGREGAR PIE DE FOTO}
    \begin{figure}[h]
  ...
  \section {Estructura del trabajo}
    ...
    de estilos artisticos en fotografias.\\
...
%https://en.wikipedia.org/wiki/Machine_learning
    ...
    \subsection{Modelos}
      ...
      predicciones lo suficientemente precisas sobre nuevos ejemplos. A continuación se enumeran algunas propiedades de los modelos:
      \begin{itemize}
	...
	\subsubsection{Función de Puntuación}
	  ...
	  de la predicción. Para el caso de un problema de clasificación de imagenes, el modelo dado una imagen, puede otorgar un puntaje a cada una de las clases posibles, 
	  ...
	  \begin{equation*}
	    f(x_{i}, W, b) = W x_{i} + b
	  \end{equation*}
	  Donde $W \in R^{KxD}$, $b \in R^{Kx1}$ son los parámetros de la función. $W$ suele ser llamado los pesos y $b$ el sesgo.  
	  
	\subsubsection{Función de Pérdida}
	  Una función de pérdida es una función real, no negativa $L(y , y)$, que mide cuán diferente es la predicción $y$ obtenida por la hipótesis $h$ , con respecto a la
	  ...
	  \paragraph{Función de Pérdida 0-1} \FIXME{AGREGAR FORMULA}
	  \paragraph{Función de Pérdida Hinge} \FIXME{AGREGAR FORMULA}
	  \paragraph{Función de Pérdida Logistica} \FIXME{AGREGAR FORMULA}
	  \paragraph{Función de Pérdida de Entropia Cruzada} \FIXME{AGREGAR FORMULA}
 ...
 \FIXME{Ver Bishop Cap 5 pag 242}
  ...
  \section {Redes Neuronales Convolucionales}
	...
	todas partes. Existen 3 generalizaciones sobre esta, que agregan un parámetro de regularización:
	  ...
	  \subparagraph{Leaky ReLU} Agrega un parametro de regularizacion, fijandolo en un valor pequeño como por ejemplo 0.01. \FIXME{AGREGAR FORMULA}
	...
	\subsubsection{Matriz de Gramm}
	  En el álgebra lineal, la matriz de Gram de un conjunto de vectores ${\ displaystyle v_ {1}, \ dots, v_ {n}} v_ {1}, \ dots, v_ {n}$ en un espacio de producto interno 
	  es la matriz hermitiana de Los productos internos, cuyas entradas están dadas por ${\ displaystyle G_ {ij} = \ langle v_ {i}, v_ {j} \ rangle} G_ {ij} = \ langle v_ {i},
	  v_ {j} \ rangle$
...
\chapter{Algoritmo de Transferencia de estilo}
    \section{Contexto}
      En este capitulo se desarrollan los contenidos específicos relativos al Algoritmo de Transferencia de Estilos artisticos a fotografias. El algoritmo elegido para tal fin es el que 
    ...
    \section{Sintesis}
      ...
      del contenido de una imagen y la respresentacion del estilo de la otra.\\
      	  \begin{figure}[h]
	    \includegraphics[scale=0.5]{./img/gatys_1.png}
	    \caption{COMPLETAR}
	    \label{fig:stanford_challenges}
	  \end{figure}
    ...
    \section{Metodos}
      ...
      \FIXME{AGREGAR FORMULA}\\
      \begin{figure}[h]
	\includegraphics[scale=0.5]{./img/gatys_2.png}
	\caption{COMPLETAR}
	\label{fig:stanford_challenges}
      \end{figure}
