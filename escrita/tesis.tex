...
\chapter{Algoritmo de Transferencia de estilo}
	...
	A continuación se enumera la lista de hiperparámetros que requiere el algoritmo para ejecutarse y su influencia:
	\begin{itemize}
	 \item Numero de iteraciones: Determina cuantas veces se ejecuta el metodo de descenso por el gradiente para minimizar la funcion de perdida. 
	  A medida que este numero aumenta, mayores son las chances de obtener un resultado aceptado.
	 ...
	 \item Factores de peso de cada capa para la representación de estilo ($w_l, l \in \{1 \dots L\}$): Por lo general, a todas las capas elegidas se les asignada
	  el mismo valor, y a las que no fueron elegidas se les asigna 0.
	 \item Factores de peso para las funciones de perdida de estilo y contenido en la funcion de perdida total $(\alpha, \beta)$: Esto determina el grado de influencia
	  de cada funcion de perdida, usualmente el ratio $\alpha / \beta$ suele estar entre $1 x 10^{-3}$ y $1 x 10^{-4}$ \FIXME{EXISTE UNA FIGURA EN gatys cvpr donde se discute esto}
	 ...
	 de contenido o la de estilo. \FIXME{EXISTE UNA FIGURA EN gatys cvpr donde se discute esto}
	\end{itemize}
