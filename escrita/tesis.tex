	...
	\subsubsection{Funciónes de Pérdida}
	  El problema de estimar los parámetros de un modelo, se resuelve optimizando una función de costo que se puede expresar como:
	    ...
	    L = \frac{1}{N}\sum_{i=1}^{N} L_i(y_i, f(x_i, W, b)) + \lambda \Omega(W)
          ...
          Esta ecuacion contiene 2 terminos: Una función de pérdida $L_i(y_i, f(x_i, W, b))$
...
\chapter{Algoritmo de Transferencia de estilo}
    ...
    \section{Sintesis \label{sec:sintesis}}
      ...
      Otro hiperparametro que requiere el algoritmo es definir cuales seran las capas de la red utilizadas para calcular el estilo y cual sera la capa elegida 
...
\chapter{Elección Automatica de hiperparámetros para Transferencia de estilo}
  \section{Descripción del problema}
    Basado en el algoritmo de transferencia de estilo se pueden obtener resultados muy interesantes, sin embargo, como se menciona en la seccion \ref{sec:sintesis} 
    es necesario definir el numero de iteraciones como un hiperparámetro, que necesita el algoritmo para poder ejecutarse. 
    El criterio de elección de este hiperparámetro termina siendo muy influyente en el resultado final.
    Con respecto a las capas elegidas para calcular el estilo y el contenido, estas fueron predefinidas en todos los casos analizados, de la misma forma, 
    para concentrarse unicamente en la elección del numero de iteraciones.
  ...
  \section{Solución propuesta \label{sec:solucion}}
	...
	A partir del puntaje de clasificacion obtenido, segun el criterio de seleccion se define si el modulo generador debe seguir ejecutandose o el resultado generado
	ya obtuvo un puntaje aceptable en el estilo objetivo.

    \subsection{Criterio de Selección}
      El criterio de selección que se definio para elegir la imagen óptima fue el siguiente:
      ...
      Una vez definido el estilo objetivo, se definen los intervalos donde el puntaje obtenido por el estilo objetivo supera al puntaje del resto de los estilos.
      La imagen optima elegida es la correspondiente al numero de iteraciones medio del intervalo donde el estilo objetivo es el maximo. En caso de que haya mas de un
      de un intervalo que cumpla con este requisito, se devuelve una imagen optima de cada intervalo como posibles imagenes optimas para que el usuario elija cual es
      la optima segun su criterio cualitativo.
...
\section{Módulo de reconocimiento}
  ...
  \subsection{Conjunto de datos}
	...
	\begin{figure}
	  ...
	  \caption{Many figures}\label{mosaico_estilos}
	\end{figure}
	...
	\begin{enumerate}
	 ...
	 \item Verificar que la descarga haya sido exitosa y el archivo obtenido este en condiciones
	 \item Ordenar los archivos segun el estilo al que pertenecen.
	\end{enumerate}

    \subsection{Detalles de implementación}
      Para realizar el ajuste fino, sobre este conjunto de datos generado, se utilizo la arquitectura de la red AlexNet, reemplazando la ultima capa por una capa que 
      ...
      Para reentrenar la red ajustando los valores para el reconocimiento de estilo, se le proveyo el conjunto de datos generado, y al cabo de 50000 iteraciones la red obtuvo un 92 \% de precisión
      en el conjunto de entrenamiento al reconocer estilos.
...
\subsection{Número de iteraciones vs. score de clasificación}
  ...
  Inicialmente se suponia que el estilo de la imagen resultante se iba a corresponder con el estilo de la imagen de contenido en todos los casos, pero luego de analizar varios casos, pudimos concluir que 
  esta hipotesis es falsa, ya que la imagen de contenido tambien influye aunque en mucho menor proporcion para la clasificacion de estilo. Lo que avala la idea que presenta Gatys de que 
  el estilo y el contenido de una imagen no son absolutamente separables.
